...
def parseBracketedExpression(text, index):
    if text[index] == "[":
        ...
        n = 0
        while n < len(text) and text[index + n] in [
            "1",
            "2",
            "3",
            "4",
            ...
            "8",
            "9",
        ]:  # this should always just happen once if input is a bracketed expression of
            # letters
            n = n + 1
...
def multiplyTensor(a, b):
    ...
    for leftLevel in range(1, level):
        for rightLevel in range(1, 1 + level - leftLevel):
            sum[leftLevel + rightLevel - 1] += numpy.outer(
                a[leftLevel - 1], b[rightLevel - 1]
            ).flatten()
    return sum
...
class TestCase(unittest.TestCase):
    if output_timings:
        ...
        def tearDownClass(cls):
            print(
                "\n%s.%s: %.3f"
                % (cls.__module__, cls.__name__, time.time() - cls.startTime)
            )
...
class A(TestCase):
    def consistency(self, coropa, dim, level):
        ...
        fullLogSig = numpy.concatenate(
            logTensor(splitConcatenatedTensor(sig, dim, level))
        )
    ...
    def testjoining(self):
        ...
        for fixedPoint, inputDim, fixed in [
            (float("nan"), dim, False),
            (0.1, dim - 1, True),
        ]:
            pathLength = 10
            ...
            calculated = iisignature.sigjoinbackprop(
                derivsOfSum, joinee, extra, level, fixedPoint
            )
            ...
            if fixed:
                ...
                bump3 = numpy.sum(
                    iisignature.sigjoin(joinee, extra, level, bumpedFixedPoint)
                )
                diff3 = (
                    bump3
                    - base
                    - numpy.sum(calculated[2] * (bumpedFixedPoint - fixedPoint))
                )
...
class LogSig2Sig(TestCase):
    def doL2S(self, coropa):
        ...
        manualChange = fdDeriv(
            lambda x: iisignature.logsigtosig(x, s), logsig, bumpedLogSig - logsig, 6
        )
        ...
        calculated = iisignature.logsigtosigbackprop(derivsOfSum, logsig, s)
        manualChange = fdDeriv(
            lambda x: iisignature.logsigtosig(x, s) * random,
            logsig,
            bumpedLogSig - logsig,
            4,
        )
...
class Deriv(TestCase):
    def testSig(self):
        ...
        path = numpy.cumsum(
            2 * (path - 0.5), 0
        )  # makes it more random-walk-ish, less like a scribble
        increment = 0.01 * numpy.random.uniform(size=(pathLength, d))
        base_sig = iisignature.sig(path, m)

        target = fdDeriv(
            lambda x: iisignature.sig(x, m), path, increment, 2, nosum=True
        )
        ...
        niceOnes = numpy.abs(calculated) > 1.0e-4
        niceOnes2 = numpy.abs(calculated) < numpy.abs(base_sig)
        diffs1 = numpy.max(
            numpy.abs((calculated[niceOnes2] - target[niceOnes2]) / base_sig[niceOnes2])
        )
    ...
    def logSig(self, type, m=7):
        ...
        manualChange = fdDeriv(
            lambda x: iisignature.logsig(x, s, type), path, increment, 4
        )

        dFdlogSig = numpy.ones(
            iisignature.siglength(d, m)
            if "X" == type
            else iisignature.logsiglength(d, m)
        )
        calculatedChange = numpy.sum(
            increment * iisignature.logsigbackprop(dFdlogSig, path, s, type)
        )
...
class SimpleCases(TestCase):
    def testFewPoints(self):
        ...
        self.assertTrue(
            numpy.array_equal(iisignature.sig(path1, m, 2), numpy.zeros([0, length]))
        )
        if CANCOMPILE:
            self.assertLess(
                diff(iisignature.logsig(path1, s, "C"), blankLogSig), 0.000000001
            )
        self.assertLess(
            diff(iisignature.logsig(path1, s, "O"), blankLogSig), 0.000000001
        )
        self.assertLess(
            diff(iisignature.logsig(path1, s, "S"), blankLogSig), 0.000000001
        )
        self.assertLess(diff(iisignature.logsig(path1, s, "X"), blankSig), 0.000000001)
        self.assertLess(
            diff(iisignature.logsig(path1, s_a, "A"), blankLogSig_a), 0.000000001
        )
        ...
        if CANCOMPILE:
            self.assertLess(
                diff(iisignature.logsig(path2, s, "C"), blankLogSig), 0.000001
            )
        ...
        self.assertLess(
            diff(iisignature.logsig(path2, s_a, "A"), blankLogSig_a), 0.000001
        )
    ...
    def testLevel1(self):
        ...
        for type_ in methodfilter(("C", "O", "S", "X", "A")):
            self.assertLess(
                diff(iisignature.logsig(path, s, type_), rightSig), 0.0000001, type_
            )
        ...
        pathderivs[0] = -derivs
        self.assertLess(
            diff(iisignature.logsigbackprop(derivs, path, s), pathderivs), 0.00001
        )
        self.assertLess(
            diff(iisignature.logsigbackprop(derivs, path, s, "X"), pathderivs), 0.00001
        )
        self.assertLess(
            diff(iisignature.sigbackprop(derivs, path, m), pathderivs), 0.00001
        )
    ...
    def testCumulative(self):
        ...
        expected = numpy.array(
            [iisignature.sig(path[: (i + 1)], m) for i in range(1, length)]
        )
    ...
    def test_big_siglength(self):
        self.assertEqual(iisignature.siglength(9, 9), sum(9**i for i in range(1, 10)))
...
class Batching(TestCase):
    def test_batch(self):
        ...
        backjoined1315 = iisignature.sigjoinbackprop(
            joined1315, sigArray1315, dataArray1315, m
        )
        self.assertEqual(backjoined1315[0].shape, sigArray1315.shape)
        self.assertEqual(backjoined1315[1].shape, dataArray1315.shape)
        self.assertTrue(
            numpy.allclose(backjoined1315[0].reshape(n, -1), backjoinedArrays[0])
        )
        self.assertTrue(
            numpy.allclose(backjoined1315[1].reshape(n, -1), backjoinedArrays[1])
        )

        dataAsSigs = [
            iisignature.sig(numpy.vstack([numpy.zeros((d,)), i]), m) for i in data
        ]
        ...
        backcombined1315 = iisignature.sigcombinebackprop(
            joined1315, sigArray1315, dataArrayAsSigs1315, d, m
        )
        backcombined = [
            iisignature.sigcombinebackprop(i, j, k, d, m)
            for i, j, k in zip(joined, sigs, dataAsSigs)
        ]
        ...
        self.assertTrue(
            numpy.allclose(backcombined1315[0].reshape(n, -1), backcombinedArrays[0])
        )
        self.assertTrue(
            numpy.allclose(backcombined1315[1].reshape(n, -1), backcombinedArrays[1])
        )
        ...
        backscaled1315 = iisignature.sigscalebackprop(
            scaled1315, sigArray1315, dataArray1315, m
        )
        self.assertEqual(backscaled1315[0].shape, sigArray1315.shape)
        self.assertEqual(backscaled1315[1].shape, dataArray1315.shape)
        self.assertTrue(
            numpy.allclose(backscaled1315[0].reshape(n, -1), backscaledArrays[0])
        )
        self.assertTrue(
            numpy.allclose(backscaled1315[1].reshape(n, -1), backscaledArrays[1])
        )
        ...
        for type in ("c", "o", "s", "x", "a", "ch", "oh", "sh", "ah"):
            ...
            self.assertEqual(
                logsigArray1315.shape, (1, 3, 1, 5, logsigs[0].shape[0]), type
            )
            self.assertTrue(
                numpy.allclose(logsigArray1315.reshape(n, -1), logsigArray), type
            )
            ...
            if type in ("s", "x", "sh"):
                ...
                self.assertTrue(
                    numpy.allclose(backlogs1315.reshape(n, 6, d), backlogs), type
                )
        ...
        for s in s_s:
            ...
            backls2s = stack(
                [
                    iisignature.logsigtosigbackprop(i, j, s)
                    for i, j in zip(sigs, logsigs)
                ]
            )
            ...
            self.assertTrue(
                numpy.allclose(backls2s1315.reshape(-1, logsigs[0].shape[0]), backls2s)
            )
...
class RotInv2d(TestCase):
    def dotest(self, type):
        ...
        angles[0] = 0
        rotationMatrices = [
            numpy.array([[math.cos(i), math.sin(i)], [-math.sin(i), math.cos(i)]])
            for i in angles
        ]
        ...
        if 1:
            ...
            lowerRotinvs = (
                0
                if 2 == m
                else iisignature.rotinv2dlength(
        ...
        if type == "a":
            ...
            totalUnspannedDimensions = sum(
                i.shape[1] - numpy.linalg.matrix_rank(i, rank_tolerance) for i in splits
            )
    ...
    def testConsistencyOfBases(self):
        ...
        for c, name in ((cs, "s"), (ck, "k"), (ca, "a"), (cq, "q")):
            ...
            self.assertLess(
                numpy.max(evil), numpy.min(odious), "bad order of rows in " + name
            )
